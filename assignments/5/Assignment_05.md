...
## Part 1: Refine the Product Backlog and Plan sprint 4
...
4. Create an explicit sprint 4 milestone with:

   * Sprint start and finish dates
   * Sprint Goal
   * Selected Sprint PBIs

5. Use the sprint 4 milestone as the authoritative Sprint container. Issues assigned to that milestone are the selected Sprint Backlog items.
...
12. The Sprint Backlog platform board/view must include the items assigned to the sprint 4 milestone and show useful work-management information where the platform supports it, such as priority, `MVP version`, estimate or Story Points, assignee, and status.
...
2. Create or update PBIs for feedback points that the team decides to address in sprint 4.
...
6. If sprint 4 changes the architecture, critical modules, deployment model, workflow, or CI configuration, update the Definition of Done so it still describes the current completion standard.
...
4. Add at least two new UAT scenarios for newly implemented `MVP v2` functionality. These scenarios must cover new or changed user-facing behavior delivered in sprint 4, even if other Sprint work focused on architecture, maintainability, deployment, or workflow improvements.
